...
/*   Updated: 2025/09/01 23:59:40 by rgohrig          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	h_set_stop_sim(t_data *data)
{
	pthread_mutex_lock(&data->stop_sim_mtx);
...
static int	h_stop_check(t_data *data)
{
	...
	while (idx < data->total_philos)
	{
		...
		if (get_time_ms(&data->eat_timestamps[idx]) >= data->die_ms)
			return (h_set_stop_sim(data),
